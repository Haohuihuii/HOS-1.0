...
%endif
    ...
    ; ds/es/fs/gs：4 * 4 = 16 Byte
    ; pushad：8 * 4 = 32 Byte
    ; 总共 48 Byte
    ;
    ; 所以此时：
    ; [esp + 48] = CPU 压入的真实 error code
    ;              或我们手动压入的 ErrCodeMagic

    ; C 调用约定：参数从右向左压栈
    ;
    ; handler(vector, errorCode)
    push dword [esp + 48]        ; 第二个参数：errorCode
    push %1                      ; 第一个参数：vector

    call [InterruptHandlerList + %1 * 4]

    ; 删除刚才传给 C 函数的两个参数
    ; vector 4 Byte + errorCode 4 Byte
    add esp, 8
